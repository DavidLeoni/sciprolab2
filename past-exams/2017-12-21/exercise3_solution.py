...
class DiGraph:
    ...
    def spantree(self, source):
        ...
        queue = Queue()        
        queue.put(source)
        visit.log(source).path = ()  # we introduce a new field 'path'
        ...
        while not queue.empty():
            ...
            if not visit.is_discovered(vertex):
                ...
                for neighbor in self.adj(vertex):                                
                    ...
                    if neighbor_log.parent == None and neighbor != source:
                        neighbor_log.parent = vertex
                        neighbor_log.path = visit.log(vertex).path + (vertex, )
                    queue.put(neighbor)                    
        
        ret = {}
        
        # Since some node may never be reached, we first we 
        # initialize everything with empty tuple: 
        for vertex in self._edges:
            ret[vertex] = tuple()     

        # --- then we overwrite with paths from the discovered logs:
            
        for log in visit.logs():
            ret[log.vertex] = log.path            
